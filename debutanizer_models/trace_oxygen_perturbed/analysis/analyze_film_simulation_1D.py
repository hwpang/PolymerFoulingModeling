...
for ind, tray in enumerate(selected_trays):
    ...
    masses = np.array([simulation.loc[len(simulation.index)-1, f"mass_cell_{cell_ind}"] for cell_ind in range(num_cells)])
    Vsolidinfilm = masses / rho
    Vliqinfilm = Vsolidinfilm / (1 - epsilon) * epsilon
    Vfilm = Vsolidinfilm + Vliqinfilm
    zs = np.cumsum(Vfilm / A)
    concs = np.array([simulation.loc[len(simulation.index)-1, f"AR_cell_{cell_ind}"] for cell_ind in range(num_cells)]) / Vsolidinfilm
