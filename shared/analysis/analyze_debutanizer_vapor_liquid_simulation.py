...
def parse_arguments():
    ...
    parser.add_argument(
        "--simulation_results_directory", type=str, required=True, help="The path to the csv file containing the simulation results.",
    )
    parser.add_argument(
...
liquid_simulation_results_path = os.path.join(simulation_results_directory, f"simulation_vapor_liquid_yliqn_{tf_liq}.csv")
...
liquid_rop_results = {}
for tray in trays:
    path = os.path.join(simulation_results_directory, f"simulation_vapor_liquid_liqrop_{tray}.csv")
...
print("Plotting radical concentrations, chemical lifetime to residence time ratio, consumption rates, and production rates...")
...
max_conc_liq_radical_labels = set([label for tray in trays for label in get_max_conc_liq_radical_label(liquid_simulation_df, tray, R_labels + ROO_labels + RO_labels, N=1)])
...
def plot_liquid_rop(name, labels, loss_only=False, production_only=False):
    ...
    fig, axs = plt.subplots(nrows=len(selected_trays), ncols=1, figsize=(9, 12), sharex=True)
    ...
    fig.savefig(f"Figures/{model_name}_liquid_rop_{label}_{name}.pdf", bbox_inches="tight")
