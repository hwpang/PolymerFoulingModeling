...
max_conc_liq_radical_labels = set([get_max_conc_liq_radical_label(liquid_simulations[perturb_factor], R_labels + ROO_labels + RO_labels) for perturb_factor in perturb_factor_list])
print("max_conc_liq_radical_labels: ", max_conc_liq_radical_labels)

fig, axs = plt.subplots(1, 1, figsize=(4, 3))
ax = axs
for label in max_conc_liq_radical_labels:
    ax.plot(xs, [get_last_row(liquid_simulations[perturb_factor], label) / Vliq for perturb_factor in perturb_factor_list], marker="o", label=label)
ax.set_yscale("log")
...
def plot_liquid_rop(name, labels, loss_only=False, production_only=False):
    ...
    for ind, perturb_factor in enumerate(perturb_factor_list):
        rops, rop_sourcestrings = get_liquid_rops(liquid_rop_results[perturb_factor], labels, radicals_only=True, loss_only=loss_only, production_only=production_only)
    ...
    fig.tight_layout()
    fig.savefig(f"Figures/QCMD_cell_model_liquid_rop_{label}_{name}.pdf", bbox_inches="tight")
